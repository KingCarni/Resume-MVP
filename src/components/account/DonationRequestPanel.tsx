...
function formatRequestStatus(rows?: DonationRequestRow[], loading = false): string {
  if (loading) return "Checking status";
  if (!rows || rows.length === 0) return "No requests made";
    ...
    <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-5 shadow-lg shadow-black/15">
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-4">
          <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Requested credits</div>
          <input
            type="number"
            min={5}
            max={200}
            ...
            className="mt-4 w-full rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-sm text-white outline-none transition focus:border-cyan-400/40"
          />
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Request status</div>
              <div className="mt-3 text-4xl font-bold tracking-tight text-white">
                {rows && rows.length > 0 ? rows[0]?.requestedCredits ?? 0 : 0}
              </div>
            </div>
            <div className="text-right text-sm font-medium text-slate-400">
              {formatRequestStatus(rows, loadingList)}
            </div>
          </div>
          <p className="mt-3 text-sm leading-7 text-slate-300">
            {formatRequestStatus(rows, loadingList) === "No requests made"
              ? "No requests have been submitted yet."
              : formatRequestStatus(rows, loadingList) === "Waiting for response"
                ? "Your latest request is waiting for review."
                : formatRequestStatus(rows, loadingList) === "Approved"
                  ? "Your latest request has been approved."
                  : "Your latest request was denied."}
          </p>
        </div>
      </div>
